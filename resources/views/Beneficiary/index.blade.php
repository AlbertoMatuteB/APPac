            ...
            <div class="bg-default-grey w-full rounded-xl grid grid-rows-2 grid-cols-4 gap-x-6 gap-y-2 items-center py-4 pr-6">
                ...
                <div>
                    <form method="POST" action="/beneficiarios/search" class="pl-4 items-center flex flex-row">
                        @csrf
                        <span class="z-10 leading-snug font-normal absolute text-center text-slate-400 absolute bg-transparent rounded text-base items-center justify-center w-8 pl-2 py-1">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                            </svg>
                        </span>
                        <input type="text" placeholder="Buscar..." id="searchnombre" name="search" class="bg-white px-3 py-2 placeholder-slate-400 text-slate-600 relative text-base  border-2 rounded-l-lg outline-none focus:border-slate-300 w-full pl-8" />
                        <button type="submit" class="rounded-r-lg  bg-slate-300 hover:bg-slate-400 text-white text-center py-2 px-4">
                            <svg class="w-6 h-6" fill="bg-blue-appac" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                ...
                <div>
                    <form method="POST" action="/beneficiarios/search/age" class="items-center flex flex-row">
                        ...
                        <input type="text" placeholder="Buscar..." id="searchage" name="search" class="bg-white px-3 py-2 placeholder-slate-400 text-slate-600 relative text-base  border-2 rounded-l-lg outline-none focus:border-slate-300 w-full pl-8" />
                        <button type="submit" class="rounded-r-lg bg-slate-300 hover:bg-slate-400 text-white text-center py-2 px-4">
                            <svg class="w-6 h-6" fill=".bg-blue-appac" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clip-rule="evenodd"></path>
                            </svg>
                ...
                <div>
                    <form method="POST" action="/beneficiarios/search/diagnostic" class="items-center flex flex-row">
                        @csrf
                        <select class="block appearance-none w-full bg-white border-gray-200 placeholder-slate-400 text-slate-600 py-2 pl-3 border-2 rounded-l-lg leading-tight focus:outline-none focus:border-slate-300" id="searchdiagnosis" name="search">
                            ...
                            <td class="p-4">
                                <div class="group inline-block relative">
                                    ...
                                    <ul class="pt-2 absolute hidden group-hover:block z-10">
                                        <li class="z-10">
                                            <a class="z-10 bg-white border hover:bg-gray-200 font-light text-lg py-2 px-4 block whitespace-no-wrap" href="{{url('/beneficiarios/'.$beneficiary->id)}}">
                                                Consultar</a>
                                        </li>
                                        <li class="z-10">
                                            <a class="z-10 bg-white border hover:bg-gray-200 font-light text-lg py-2 px-4 block whitespace-no-wrap" href="{{url('/evaluaciones/crear/'.$beneficiary->id)}}">
                                                Evaluar</a>
                                        </li>

                                        <li class="z-10">
                                            <a class="z-10 bg-white border hover:bg-gray-200 font-light text-lg py-2 px-4 block whitespace-no-wrap" href="{{url('/beneficiarios/'.$beneficiary->id.'/edit')}}">
                                                Editar</a>
                                        </li>

                                        <li class="z-10">
                                            <a class="z-10 bg-white border hover:bg-gray-200 font-light text-lg py-2 px-4 block whitespace-no-wrap" @click=" isModalOpen = true ">
                                                Elminiar
                                            </a>
                                            {{-- <a class="z-10 bg-white border hover:bg-gray-200 font-light text-lg py-2 px-4 block whitespace-no-wrap">
                                                ...
                                                @csrf
                                                    <input type="submit" onclick="return confirm('¿Quiere Eliminar Beneficiario?')" class="btn btn-outline-danger" value="Eliminar" @click=" isModalOpen = true ">
                                                </form>
                                            </a> --}}
                                        </li>
                                    </ul>
